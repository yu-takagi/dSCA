...
for ct1 = 1:T
    for ct2 = ct1+1:T
        ...
        [err_a2b_cca_noave(ct1,ct2),cc_a2b_noave(ct1,ct2)] =cca_cv_noave(fr_a_org(:,:,:,t1,:), fr_b_org(:,:,:,t2,:), size(fr_a_org,ndims(fr_a_org)), ...
                        'numCompsCCA',1, 'numCompsPCA', 0, 'numRep', 15,'simultaneous',true,'zscore',true, 'lambda',0.05);
...
for ct1 = 1:T
    for ct2 = ct1+1:T
        ...
        [err_a2b_rrr_noave(ct1,ct2)] =rrr_cv_noave(fr_a_org(:,:,:,t1,:), fr_b_org(:,:,:,t2,:), size(fr_a_org,ndims(fr_a_org)), ...
                        'numCompsRRR',1, 'numCompsPCA', 0, 'numRep', 15,'simultaneous',true,'zscore',true, 'lambda',0.05);
...
save(['./results/simulation_',num2str(seed,'%04d')],'out')

%% Make figures
figure('Renderer', 'painters', 'Position', [10 10 1000 260])
subplot(1,4,1)
im = cc_a2b_noave+cc_b2a_noave';
im(logical(eye(size(im)))) = NaN;
imagesc(im,[0.4,1])
title('CCA (Raw) CanonCorr')
xlabel('Time of a')
ylabel('Time of b')
set(gca,'YDir','normal')
colormap hot
...
subplot(1,4,2)
im = err_a2b_rrr_noave+err_b2a_rrr_noave';
im(logical(eye(size(im)))) = NaN;
imagesc(1-im, [0,0.5])
title('RRR (Raw) ExpVar')
xlabel('Time of a')
ylabel('Time of b')
set(gca,'YDir','normal')
colormap hot
